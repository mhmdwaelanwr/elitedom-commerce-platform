"""Stage 8 catalogue content, attributes, publication, and media service."""
...
class CatalogContentService:
    """A content layer around the inventory-safe ProductTemplate SKU master."""
    ...
    async def list_public_products(
        ...
        *,
        ...
        joined = (
            ...
            .outerjoin(
                ProductCatalogContent,
                ProductCatalogContent.product_id == ProductTemplate.id,
            )
        ...
        products = list(
            (
                await self.db.scalars(
                    ...
                    .order_by(
                        ProductCatalogContent.is_featured.desc().nullslast(),
                        ProductTemplate.updated_at.desc().nullslast(),
        ...
        hydrated = await self._hydrate_products(products, locale=locale)
    ...
    async def get_public_product(
        ...
        *,
        ...
        if identifier.isdigit():
            ...
            query = query.where(ProductCatalogContent.slug == self._slugify(identifier))
    ...
    async def list_public_categories(
        ...
        *,
        ...
        content_by_id = await self._category_content_map(category.id for category in categories)
        nodes = {
            category.id: self._category_response(
                category,
                content_by_id.get(category.id),
                locale=locale,
            )
    ...
    async def update_product_content(
        ...
        return await self.get_admin_product_content(product.id)
    ...
    async def list_admin_categories(self) -> list[CatalogCategoryAdminResponse]:
        ...
        content_by_id = await self._category_content_map(category.id for category in categories)
        return [
            self._category_admin_response(category, content_by_id.get(category.id))
            for category in categories
        ]
    ...
    async def create_category(
        ...
        slug = self._slugify(request.slug)
    ...
    async def list_attribute_definitions(self) -> list[CatalogAttributeDefinitionResponse]:
        ...
        return [CatalogAttributeDefinitionResponse.model_validate(item, from_attributes=True) for item in definitions]
    ...
    async def replace_product_attributes(
        ...
        if set(ids) != set(definition_by_id):
            raise ResourceNotFoundError("Product attribute", "unknown")
        for item in request.attributes:
            definition = definition_by_id[item.attribute_id]
            self._validate_attribute_value(definition, item)
        ...
        await self.db.flush()
        await self._publish_product_event(product)
        return await self._load_attributes([product_id], locale="en").then(product_id)  # type: ignore[attr-defined]
    ...
    async def reorder_media(
        ...
        requested_ids = {item.image_id for item in request.images}
        if requested_ids != set(image_by_id):
            raise ResourceConflictError("Media ordering must include every image attached to the product.")
        metadata_by_id = await self._media_metadata_map(image_by_id)
    ...
    async def _hydrate_products(
        ...
        *,
        ...
        attribute_map = await self._load_attributes(product_ids, locale=locale)
        image_ids = [image.id for product in products for image in product.images]
        media_metadata = await self._media_metadata_map(image_ids)
        ...
        for product in products:
            ...
            name = self._localized(content.name_ar if content else None, product.name, locale)
            ...
            images = [
                self._image_response(image, media_metadata.get(image.id), locale=locale)
                for image in sorted(
                    product.images,
    ...
    async def _load_attributes(
        ...
        *,
        ...
        for value, definition in rows:
            rendered_value = self._render_attribute_value(value, definition, locale)
            result.setdefault(value.product_id, []).append(
                CatalogAttributeResponse(
                    definition_id=definition.id,
                    code=definition.code,
                    label=self._localized(definition.name_ar, definition.name, locale),
    ...
    async def _ensure_product_content(
        ...
        *,
        ...
        query = select(ProductCatalogContent).where(
