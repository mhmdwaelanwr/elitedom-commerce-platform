...
type AdminPlatformPageProps = { kind: AdminPlatformKind };
...
const config: Record<AdminPlatformKind, {
  icon: StoreIconName;
  ...
  payments: {
    ...
    title: "Payments",
    titleAr: "المدفوعات",
    description: "Provider-neutral settlement, attempts and audited refund requests.",
    descriptionAr: "التسويات ومحاولات الدفع وطلبات الاسترداد المراجعة بدون كشف اسم البوابة للعميل.",
    figmaNode: "244:296",
  },
  inventory: {
    icon: "warehouse",
    ...
    title: "Inventory",
    titleAr: "المخزون",
    description: "Persisted stock health, low-stock exceptions and catalogue coverage.",
    descriptionAr: "حالة المخزون الفعلية وتنبيهات النقص وتغطية الكتالوج من السجلات الحقيقية.",
    figmaNode: "244:589",
  },
  suppliers: {
    icon: "building",
    ...
    description: "Verified suppliers, purchase orders and delivery performance.",
    descriptionAr: "الموردون الموثقون وأوامر الشراء وأداء التسليم.",
    figmaNode: "244:882",
  },
  reports: {
    icon: "file",
    ...
    description: "Operational sales, inventory and RMA reporting from persisted records.",
    descriptionAr: "تقارير المبيعات والمخزون وRMA من السجلات المحفوظة فعليًا.",
    figmaNode: "244:1175",
  },
  catalog: {
    icon: "edit",
    ...
    titleAr: "محرر الكتالوج",
    description: "Category and technical-attribute definitions powering the public catalogue.",
    descriptionAr: "تعريفات الأقسام والخصائص التقنية التي تغذي الكتالوج العام.",
    figmaNode: "244:589",
  },
  integrations: {
    icon: "plug",
    permission: "integrations.view",
    eyebrow: "INTEGRATIONS & AUDIT",
    title: "Integrations",
    titleAr: "التكاملات",
    description: "Runtime readiness plus immutable audit evidence. External provider health stays release-scoped.",
    descriptionAr: "جاهزية التشغيل مع سجل تدقيق ثابت؛ صحة المزود الخارجي تظل مرتبطة بأدلة الإصدار.",
    figmaNode: "244:2347",
  },
};
...
export function AdminPlatformPage({ kind }: AdminPlatformPageProps) {
  ...
  const loadData = useCallback(async (current: CustomerSession) => {
    if (kind === "payments") {
      const orders = await fetchAdminOrders(current, { page: 1 });
      const visible = orders.orders.slice(0, 16);
    ...
    if (kind === "inventory") {
      const [inventory, products] = await Promise.all([fetchInventoryReport(current), fetchAdminProducts(current, { page: 1 })]);
      setData({ inventory, products: products.products });
      return;
    }
    if (kind === "suppliers") {
    ...
    if (kind === "reports") {
      const [reporting, inventory, rmaReport] = await Promise.all([fetchReportingDashboard(current), fetchInventoryReport(current), fetchRmaReport(current)]);
      setData({ reporting, inventory, rmaReport });
      return;
    }
    if (kind === "catalog") {
    ...
    const [readiness, audit] = await Promise.all([fetchRuntimeReadiness(), fetchAdminAuditLogs(current, { page: 1 })]);
    setData({ readiness, audit: audit.logs });
  }, [kind]);
  ...
  if (state !== "ready" || !session || !access) {
    return <PlatformGate locale={locale} state={state} error={error} />;
  }
...
function PlatformBody({ kind, data, locale, ar, access, onRefund, busyOrder }: {
  kind: AdminPlatformKind;
  data: PlatformData;
  locale: "en" | "ar";
  ar: boolean;
  access: AdminAccess;
...
function PaymentsSurface({ data, locale, ar, access, onRefund, busyOrder }: { data: PlatformData; locale: "en" | "ar"; ar: boolean; access: AdminAccess; onRefund: (order: AdminOrder) => void; busyOrder: number | null }) {
  ...
  return <>
    <KpiRow items={[[ar ? "طلبات ظاهرة" : "VISIBLE ORDERS", n(orders.length, locale), ar ? "آخر سجلات" : "latest records"], [ar ? "مدفوع" : "PAID", n(paid, locale), ar ? "حالة محلية" : "local state"], [ar ? "استرداد" : "REFUNDS", n(refunding, locale), ar ? "طلبات مسجلة" : "recorded requests"], [ar ? "المحاولات" : "ATTEMPTS", n(data.paymentTrails?.length ?? 0, locale), ar ? "مسار مزود محفوظ" : "persisted trail"]]} />
    <DataPanel title={ar ? "دفتر المدفوعات" : "Payment ledger"}>
      <DataTable headers={[ar ? "الطلب" : "Order", ar ? "الدفع" : "Payment", ar ? "المحاولة" : "Attempt", ar ? "الإجمالي" : "Total", ar ? "الإجراء" : "Action"]}>
        {orders.map((order) => { const trail = trails.get(order.id); return <tr key={order.id}><td><code>#{order.order_number}</code></td><td><Status value={order.payment_status} /></td><td>{trail?.provider_attempt_status ? human(trail.provider_attempt_status) : "—"}</td><td>{money(order.amount_total, locale)} EGP</td><td>{access.permissions.includes("payments.refund") && order.payment_status === "paid" ? <button disabled={busyOrder === order.id} onClick={() => onRefund(order)} type="button">{busyOrder === order.id ? "…" : ar ? "طلب استرداد" : "Request refund"}</button> : "—"}</td></tr>; })}
      </DataTable>
    </DataPanel>
  </>;
}
...
function InventorySurface({ data, locale, ar }: { data: PlatformData; locale: "en" | "ar"; ar: boolean }) {
  ...
  return <>
    <KpiRow items={[[ar ? "عدد SKU" : "SKU COUNT", n(report?.total_sku_count ?? 0, locale), ar ? "مسجل" : "persisted"], [ar ? "وحدات متاحة" : "UNITS ON HAND", n(report?.total_units_on_hand ?? 0, locale), ar ? "إجمالي" : "total"], [ar ? "قيمة التجزئة" : "RETAIL VALUE", `${money(report?.total_retail_value_egp ?? 0, locale)} EGP`, ar ? "محلية" : "persisted"], [ar ? "مخزون منخفض" : "LOW STOCK", n(report?.low_stock_products.length ?? 0, locale), ar ? "منتج" : "SKUs"]]} />
    <DataPanel title={ar ? "تنبيهات المخزون" : "Inventory exceptions"}>
      <DataTable headers={["SKU", ar ? "المنتج" : "Product", ar ? "الكمية" : "Stock", ar ? "Dropship" : "Dropship"]}>{(report?.low_stock_products ?? []).map((product) => <tr key={product.product_id}><td><code>{product.sku}</code></td><td>{product.name}</td><td><Status value={String(product.stock_qty)} tone={product.stock_qty <= 2 ? "warning" : "accent"} /></td><td>{product.is_dropship_enabled ? (ar ? "مفعل" : "Enabled") : "—"}</td></tr>)}</DataTable>
    </DataPanel>
  </>;
}
...
function ReportsSurface({ data, locale, ar }: { data: PlatformData; locale: "en" | "ar"; ar: boolean }) {
  ...
  return <>
    <KpiRow items={[[ar ? "الإيراد" : "REVENUE", `${money(report?.total_revenue ?? 0, locale)} EGP`, ar ? "الفترة الحالية" : "selected period"], [ar ? "الطلبات" : "ORDERS", n(report?.total_orders ?? 0, locale), `${n(report?.paid_orders ?? 0, locale)} ${ar ? "مدفوع" : "paid"}`], [ar ? "العملاء النشطون" : "ACTIVE CUSTOMERS", n(report?.active_customers ?? 0, locale), ar ? "فعلي" : "persisted"], [ar ? "RMA" : "RMA CLAIMS", n(data.rmaReport?.total_claims ?? 0, locale), `${n(data.rmaReport?.recent_claims ?? 0, locale)} ${ar ? "حديث" : "recent"}`]]} />
    <div className="el-platform-grid"><DataPanel title={ar ? "سلسلة الإيراد" : "Revenue series"}><DataTable headers={[ar ? "الفترة" : "Period", ar ? "الطلبات" : "Orders", ar ? "الإيراد" : "Revenue"]}>{(report?.revenue_series ?? []).slice(-12).map((point) => <tr key={point.period}><td>{point.period}</td><td>{n(point.order_count, locale)}</td><td>{money(point.revenue, locale)} EGP</td></tr>)}</DataTable></DataPanel><DataPanel title={ar ? "الأكثر مبيعًا" : "Best sellers"}><DataTable headers={["SKU", ar ? "المنتج" : "Product", ar ? "الوحدات" : "Units"]}>{(report?.best_sellers ?? []).slice(0, 12).map((item) => <tr key={item.product_id}><td><code>{item.sku}</code></td><td>{item.name}</td><td>{n(item.units_sold, locale)}</td></tr>)}</DataTable></DataPanel></div>
  </>;
}

function CatalogSurface({ data, locale, ar }: { data: PlatformData; locale: "en" | "ar"; ar: boolean }) {
  return <>
    <KpiRow items={[[ar ? "المنتجات" : "PRODUCTS", n(data.products?.length ?? 0, locale), ar ? "العينة الحالية" : "current page"], [ar ? "الأقسام" : "CATEGORIES", n(data.categories?.length ?? 0, locale), ar ? "تعريف" : "definitions"], [ar ? "الخصائص" : "ATTRIBUTES", n(data.attributes?.length ?? 0, locale), ar ? "تقنية" : "technical"], [ar ? "فلاتر" : "FILTERABLE", n(data.attributes?.filter((item) => item.is_filterable).length ?? 0, locale), ar ? "خاصية" : "attributes"]]} />
    <div className="el-platform-grid"><DataPanel title={ar ? "الأقسام" : "Categories"}><DataTable headers={[ar ? "الاسم" : "Name", "Slug", ar ? "الحالة" : "State"]}>{(data.categories ?? []).map((item) => <tr key={item.id}><td dir="auto">{ar && item.name_ar ? item.name_ar : item.name}</td><td><code>{item.slug}</code></td><td><Status value={item.is_active === false ? (ar ? "غير نشط" : "Inactive") : (ar ? "نشط" : "Active")} tone={item.is_active === false ? "muted" : "success"} /></td></tr>)}</DataTable></DataPanel><DataPanel title={ar ? "الخصائص التقنية" : "Technical attributes"}><DataTable headers={[ar ? "الاسم" : "Name", "Code", ar ? "فلتر" : "Filter"]}>{(data.attributes ?? []).map((item) => <tr key={item.id}><td dir="auto">{ar && item.name_ar ? item.name_ar : item.name}</td><td><code>{item.code}</code></td><td>{item.is_filterable ? (ar ? "نعم" : "Yes") : "—"}</td></tr>)}</DataTable></DataPanel></div>
  </>;
}
...
function IntegrationsSurface({ data, ar }: { data: PlatformData; ar: boolean }) {
  ...
  return <>
    <KpiRow items={[["API", data.readiness?.ready ? "READY" : "CHECK", ar ? "فحص runtime" : "runtime readiness"], ["POSTGRES", deps.postgres ?? "—", ar ? "جاهزية" : "readiness"], ["REDIS", deps.redis ?? "—", ar ? "جاهزية" : "readiness"], [ar ? "التدقيق" : "AUDIT", String(data.audit?.length ?? 0), ar ? "أحداث حديثة" : "recent events"]]} />
    <div className="el-platform-grid"><DataPanel title={ar ? "جاهزية التشغيل" : "Runtime readiness"}><DataTable headers={[ar ? "الخدمة" : "Dependency", ar ? "الحالة" : "State"]}>{rows.map(([name, status]) => <tr key={name}><td>{name}</td><td><Status value={status} tone={status === "ready" ? "success" : "muted"} /></td></tr>)}<tr><td>Odoo 17</td><td><Status value={ar ? "دليل إصدار" : "Release-scoped"} tone="muted" /></td></tr><tr><td>{ar ? "مزود الدفع" : "Payment provider"}</td><td><Status value={ar ? "دليل إصدار" : "Release-scoped"} tone="muted" /></td></tr></DataTable></DataPanel><DataPanel title={ar ? "آخر أحداث التدقيق" : "Recent audit events"}><DataTable headers={[ar ? "الإجراء" : "Action", ar ? "الكيان" : "Entity", ar ? "الوقت" : "Time"]}>{(data.audit ?? []).slice(0, 12).map((item) => <tr key={item.id}><td><code>{item.action}</code></td><td>{item.entity_type}{item.entity_id ? ` #${item.entity_id}` : ""}</td><td>{new Date(item.created_at).toLocaleString()}</td></tr>)}</DataTable></DataPanel></div>
  </>;
}

function KpiRow({ items }: { items: Array<[string, string, string]> }) {
  return <section className="el-platform-kpis">{items.map(([label, value, meta]) => <article key={label}><small>{label}</small><strong>{value}</strong><span>{meta}</span></article>)}</section>;
}

function DataPanel({ title, children }: { title: string; children: ReactNode }) {
  return <section className="el-platform-panel"><h2>{title}</h2>{children}</section>;
}

function DataTable({ headers, children }: { headers: string[]; children: ReactNode }) {
  return <div className="el-platform-table-wrap"><table><thead><tr>{headers.map((header) => <th key={header}>{header}</th>)}</tr></thead><tbody>{children}</tbody></table></div>;
}

function Status({ value, tone }: { value: string; tone?: "success" | "warning" | "accent" | "muted" }) {
  const normalized = value.toLowerCase();
  const inferred = tone ?? (/(paid|ready|received|verified|active|done|success)/.test(normalized) ? "success" : /(pending|partial|low|refund|review)/.test(normalized) ? "warning" : "accent");
  return <span className={`el-platform-status is-${inferred}`}>{human(value)}</span>;
}

function PlatformGate({ locale, state, error }: { locale: "en" | "ar"; state: State; error: string }) {
  const ar = locale === "ar";
  const label = state === "loading" ? (ar ? "جارٍ تحميل سطح العمليات…" : "Loading operations surface…") : state === "denied" ? (ar ? "الحساب لا يملك الصلاحية أو يحتاج تحقق MFA." : "This account lacks permission or requires MFA verification.") : error || (ar ? "تعذر تحميل البيانات." : "The data could not be loaded.");
  return <main className="el-platform-gate" dir={ar ? "rtl" : "ltr"}><ElitedomBrand /><StoreIcon name={state === "denied" ? "lock" : "shield"} size={34} /><h1>{label}</h1>{state !== "loading" ? <Link to="/admin">{ar ? "العودة للإدارة" : "Back to admin"}</Link> : null}</main>;
}
