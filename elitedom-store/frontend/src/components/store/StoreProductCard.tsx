    ...
    <article className={`group relative flex overflow-hidden rounded-2xl border border-slate-800 bg-slate-900/70 shadow-sm transition duration-300 hover:-translate-y-1 hover:border-sky-500/60 hover:shadow-xl hover:shadow-sky-950/20 ${isList ? "flex-col sm:flex-row" : "h-full flex-col"}`}>
      ...
      <div className={`flex min-w-0 flex-1 flex-col p-5 ${isList ? "sm:p-6" : ""}`}>
        <div className="flex items-center justify-between gap-3 text-xs font-semibold">
          ...
          <span className="shrink-0 text-amber-300">★ {product.rating.toFixed(1)}</span>
        </div>
        <Link className={`mt-2 font-bold leading-6 text-white hover:text-sky-300 focus-ring ${isList ? "text-lg sm:text-xl" : "line-clamp-2 min-h-12 text-base"}`} href={`/products/${product.id}`}>
          {product.name}
        </Link>
        ...
        <div className="mt-4 flex flex-wrap gap-1.5">
          {product.specs.slice(0, isList ? 4 : 2).map((spec) => (
            <span className="rounded-md bg-slate-800 px-2 py-1 text-[10px] font-medium text-slate-300" key={spec.label}>{spec.label}: {spec.value}</span>
          ))}
        </div>
        <div className={`mt-auto flex flex-wrap items-end justify-between gap-3 border-t border-slate-800 pt-4 ${isList ? "sm:mt-5" : ""}`}>
          <div>
            <p className="text-[11px] font-medium text-slate-500">VAT included</p>
            <p className="mt-0.5 text-lg font-black text-white">{formatPrice(product.priceEgp, currency)}</p>
          </div>
          <div className="flex items-center gap-2">
            {isList && <Link className="hidden text-sm font-bold text-sky-300 hover:text-white focus-ring sm:inline" href={`/products/${product.id}`}>Details</Link>}
            <button className="button-primary shrink-0 px-3 py-2 text-sm disabled:cursor-not-allowed disabled:border-slate-700 disabled:bg-slate-800 disabled:text-slate-500" disabled={!available} onClick={() => addToCart(product)} type="button">
